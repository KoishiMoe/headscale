...
> [!NOTE]
> The server-side administration CLI commands (`headscale lock status`, `headscale lock nodes`) and their corresponding HTTP API (`/api/v1/lock`) were introduced alongside the Huma v2 REST API migration in `main`. On the `v0.29.3-tailnet-lock` release branch, Tailnet Lock operates purely via the standard Noise wire protocol endpoints (`/machine/tka/*`), and lock operations are performed directly through the official Tailscale client (`tailscale lock ...`).
